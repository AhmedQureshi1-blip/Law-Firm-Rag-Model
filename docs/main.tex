...
Retrieved chunks, the checklist question, and a structured system prompt are sent
to Claude Sonnet 4.6 (production) or Groq Llama 3.3 70B (development/testing).
The LLM responds in a structured JSON format: \texttt{finding},
\texttt{clause\_reference}, \texttt{page\_number}, \texttt{risk\_level}, and
\texttt{recommendation}. Red flag detection is integrated into the same pass.
...
\subsubsection{Groq Llama 3.3 70B (Development \& Testing)}

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{Cost:} Completely free on the developer tier---no credit card
    required, no per-token charge.
    \item \textbf{Free-tier rate limits:} 30 RPM, 1,000 RPD, 12,000 TPM, and
    100,000 TPD---sufficient for all development and demo testing workflows.
    \item \textbf{Speed:} LPU hardware delivers 300--400 tokens per second,
    making demo runs impressively fast for client presentations.
    \item \textbf{API compatibility:} OpenAI-compatible endpoint---switching to
    Claude in production requires changing only the base URL and API key. Zero
    other code changes.
\end{itemize}
...
All 5-day sprint development and testing uses Groq Llama 3.3 70B on the free
tier. \textbf{Total API development cost: PKR 0.} Free-tier limits (30 RPM,
12K TPM, 100K tokens/day) are sufficient for all testing and demo workflows.
\end{calloutbox}
...
\subsubsection{Target 1 --- Rana Ijaz \& Partners (Islamabad)}

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{Founded:} 1967 (formerly Rana Law Chambers)
    \item \textbf{Location:} Islamabad
    \item \textbf{Practice areas:} Legal due diligence, contracts, company
    incorporation, commercial litigation, domestic and international arbitration
    \item \textbf{Why a fit:} ``Legal due diligence'' is explicitly listed as a
    core practice area---a direct product-market match
    \item \textbf{Transaction type for demo:} Property due diligence
    \item \textbf{Outreach:} LinkedIn search ``Rana Ijaz Partners Islamabad'' and
    website contact form; request the Managing Partner or Senior Associate
    handling due diligence matters
\end{itemize}

\subsubsection{Target 2 --- M.A. Awan \& Co. / MAACO (Rawalpindi \& Islamabad)}

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{Location:} Headquartered in Rawalpindi and Islamabad
    \item \textbf{Practice areas:} Corporate, commercial, real estate, banking,
    tax, civil, and criminal law
    \item \textbf{Why a fit:} Covers all three target transaction types; based
    in Rawalpindi making an in-person demo logistically simple
    \item \textbf{Transaction types for demo:} Property due diligence (primary),
    loan documentation review (secondary)
    \item \textbf{Outreach:} Lawzana.com listing for direct contact details;
    call and ask for the partner handling property or banking transactions
\end{itemize}

\subsection{Backup Target}

\textbf{Mumtaz \& Brohi (Islamabad)} --- Premier firm with a team of
foreign-qualified lawyers handling transactional, advisory, and adversarial legal
